...
void ExecComd(int k)		//执行命令
{
	...
	case 25:UdTabComd();		//udtab命令，显示被删除文件表(调试程序用)
		break;
	case 26:FileCompComd(k);		//FileCompare命令
		break;
	case 27:ReplaceComd(k);		//FileCompare命令
		break;
	case 28:MoveComd(k);		//FileCompare命令
		break;
	case 29:BatchComd(k);		//FileCompare命令
		break;
	default:cout << "\n命令错:" << comd[0] << endl;
	}
}
...
int FindFCB(char* Name, int s, char attrib, FCB*& fcbp)
{
	...
	while (s > 0)
	{
		...
		for (i = 0; i < 4; i++, fcbp++)		//每个盘块4个目录项
		{
			...
			{
				if (attrib == '\040')		//attrib为32时，文件、子目录不限001  0000
					return fcbp->Addr;
				Attrib = fcbp->Fattrib;
...
int FileCompComd(int k)
{
	...
	bool FILE1_EXIST = true;
	s0 = ProcessPath(comd[1], FileName1, k, 2, '\20');
	if (s0 < 1)			//路径错误
		return s0;		//失败，返回
	ss0 = FindFCB(FileName1, s0, attrib, fcbp);
	...
	s1 = ProcessPath(comd[2], FileName2, k, 2, '\20');
	if (s1 < 1)			//路径错误
		return s1;		//失败，返回
	ss1 = FindFCB(FileName2, s1, attrib, fcbp);
	...
	int lessFcb = fcb1.Fsize < fcb2.Fsize ? fcb1.Fsize : fcb2.Fsize;

	while (count < lessFcb)
	{
		for (i = 0, j = 0; i < SIZE; i++, j++)
		{
			if (Disk[ss0][i] != Disk[ss1][i]) {
				cout << "文件"
					<< "C:" << gFileName1 << fcb1.FileName
					<< "与文件"
					<< "C:" << gFileName2 << fcb1.FileName
					<< "在第" << i + 1 << "字节处不同："
					<< Disk[ss0][i] << " , " << Disk[ss1][i];
				return -2;
			}
			count++;
			if (count == lessFcb)
...
int ReplaceComd(int k)
{
	short s, s0, s1;
	char attrib = '\0', * FileName1, * FileName2;
	char cFileName[PATH_LEN];	//目标目录文件名路径
	char gFileName[PATH_LEN];	//存放文件全路径名
	char cpath[PATH_LEN];
	FCB * fcbp1, * fcbp2;
	short i, j, count = 0;
	...
	if (k == 1) {
		...
		s = ProcessPath(comd[1], FileName1, k, 1, '\20');
		if (s < 1)
			return s;
		s0 = FindFCB(FileName1, s, attrib, fcbp1);

		//strcpy(gFileName, temppath);
		strcat(cpath, "/");
		strcat(cpath, fcbp1->FileName);
		strcpy(cFileName, cpath);

		s = ProcessPath(cFileName, FileName2, k, 1, '\20');
		if (s < 1)
			return s;
		s1 = FindFCB(FileName2, s, attrib, fcbp2);
		
		if (s1 < 0)
		{
			cout << "\n目录 " << cpath << "中没有可取代的同名文件。\n";
			return -1;
		}
		if (strcmp(FileName2, comd[1]) == 0) {
			cout << "\n文件不能自己取代自己\n";
			return -1;
		}
	...
	if (k == 2) {
		s = ProcessPath(comd[1], FileName1, k, 2, '\20');
		if (s < 1)
			return s;
		s0 = FindFCB(FileName1, s, attrib, fcbp1);
		...
		s = ProcessPath(cpath, FileName2, k, 2, '\20');
		if (s < 1)
			return s;
		s1 = FindFCB(FileName2, s, attrib, fcbp2);
	...
	if (fcbp2->Fattrib >= 2 && fcbp2->Fattrib <= 7) {
		cout << "\n文件具有系统或隐藏属性。\n";
		return -1;
	}
...
int MoveComd(int k)
{
	...
	s0 = ProcessPath(comd[1], name0, k, 2, '\040');
	if (s0 < 0) return s0;
	s1 = ProcessPath(comd[2], name1, k, 2, '\020');
	if (s1 < 0) return s1;

	ss0 = FindFCB(name0, s0, '\0', fcbp0);
	ss1 = FindFCB(name1, s1, '\020', fcbp1);
	...
	if (ss0 >= 0){
		...
		else
		{
			...
			ts = FindBlankFCB(ss1, fcbp1);
		}

		*fcbp1 = *fcbp0;						 //源文件的目录项复制给目标文件
		strcpy(fcbp1->FileName, name0); //写目标文件名
		short b0 = 0, b;
		while (ss0 > 0) //开始复制文件内容
		{
			b = getblock();
			if (b0 == 0)
				fcbp1->Addr = b; //目标文件的首块号
			else
				FAT[b0] = b;
			memcpy(Disk[b], Disk[ss0], SIZE); //复制盘块
			ss0 = FAT[ss0];				   //准备复制下一个盘块
			b0 = b;
		}

		fcbp0->FileName[0] = (char)0xe5; //删除目录项
		tmp = ss0;
		while (tmp > 0)
		{ //回收磁盘空间
			t = tmp;
		...
		char pt[PATH_LEN];
		strcpy(pt, curpath.cpath);
		strcat(pt, comd[1]);

		cout << "C:" << pt << " >> " << "C:/" << comd[2] << "/" << name0 << " [OK]" << endl;
		return 1;
	}

	ss0 = FindFCB(name0, s0, '\020', fcbp0);
	ss1 = FindFCB(name1, s1, '\040', fcbp1);
	if (ss0 < 0)
	{
		cout << "\n源目录不存在\n";
		return -1;
	...
	if (ss1 < 0) //file2不存在，则为目录改名
	{
		strcpy(fcbp0->FileName, name1);
	}
	else
	{
		short tt =FindFCB(name0, s1, '\0', fcbp1);
		if (tt > 0) {
			cout << "\n文件已存在，是否覆盖当前文件？(Y/n) ";
			char IMPUT;
			cin >> IMPUT;
		...
		tt = FindFCB(name0, s1, '\020', fcbp1);
		if (FindFCB(name0, s1, '\020', fcbp1) > 0) {
			cout << "\n目标目录已存在\n";
			return -1;
		}
		strcpy(temppath, comd[2]);
		ts = FindBlankFCB(ss1, fcbp1);
		s0 = M_NewDir(fcbp0->FileName, fcbp0, ss1, '\020');
		*fcbp1 = *fcbp0;
		fcbp0->FileName[0] = (char)0xe5;
	}
	char pt[PATH_LEN];
	strcpy(pt, curpath.cpath);
	strcat(pt,"/");
...
int BatchComd(int k) {
	...
	s = ProcessPath(comd[1], name, k, 1, '\0');
	ss = FindFCB(name, s, '\0', fcbp);
